;--------------------------------------------------------------------------------------
; Version with listview and parameter for input file.
;
...
initialize() {
    ...
    txtFileRoutines := A_Args[1] <> "" ? A_Args[1] : "cbtreef5.txt"
    ; file to process (routines code)
    txtFileCode := A_Args[2] <> "" ? A_Args[2] : "MPMDL001.CBLLE"

    user := getSystem()

    ; if (A_Args.length() < 3) {
    ;     msgbox, % "3 parameters are required:" "`n`tcbtreef5.txt" "`n`tprogramname.cblle.txt" "`n`tpath to above files"
    ;     ExitApp
    ; }

    if (user = "SYSTEM_WORK")
        path := A_Args[3] <> "" ? A_Args[3] : "H:\MY DATA\temp\projects\tree cobol program\"
    if (user ="SYSTEM_HOME")
    ...
    global ListViewWidth := 700 - TreeViewWidth - 30
    global MyTreeView, MyListView, MyEdit
    ...
    Gui, Add, ListView, -hdr  vMyListView  w%ListViewWidth% x+5, Routine name: (click any routine to show the code)
    ; LV_SetImageList( DllCall( "ImageList_Create", Int,2, Int,[color=red]20[/color], Int,0x18, Int,1, Int,1 ), 1 )
    ; Gui, guicode:Add, ListView, vMyListView r40 w%ListViewWidth% x+10, Code
...
MyTreeView:
{
    ...
    if (A_GuiEvent = "S") {
        ...
        loadListview(SelectedItemText)              ; load routine code
    }
...
GuiSize:  ; Expand/shrink the ListView and TreeView in response to user's resizing of window.
{
    ...
    GuiControl, Move, MyListView, % "H" . (A_GuiHeight - 30) . " W" . (A_GuiWidth - TreeViewWidth - 15) ; width = total - treeview - (3 X 5) margins.

    ; GuiControl, Move, MyTreeView, % "H" . (A_GuiHeight - 30) . " W" . (A_GuiWidth - ListViewWidth - 10) ; -30 for StatusBar and margins.
    
    ; change listview column width to listview width.
    LV_ModifyCol(1, A_GuiWidth - TreeViewWidth - 15 )
...
;-----------------------------------------------------------------------
populateCode() {
    Loop, Read, %filenameCode%
    {
        allCode.push(A_LoopReadLine)
    }
}
;-----------------------------------------------------------------------
; load listview with the routine code (source)
;-----------------------------------------------------------------------
loadListview(routineName) {
    ...
    while (line_number <= to_line_number) {
        LV_Add("", allcode[line_number])
        line_number ++
    }

    LV_ModifyCol(1, Auto, "Routine name:" . routineName)  ; add routine name in column
