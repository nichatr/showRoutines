...
Prism.languages.cobol = {
  ...
  callRoutine: {
    pattern: /(?<=\sperform\s)\s*[\-\w]+/im, // positive lookbehind: if [perform XXXX] --> XXXX = routine executed.
...
Prism.languages.rpg = {
  comment: [
    {
      pattern: /^[\sa-z]\*.*$/im, // m = ^ and $ match the start and end of line (otherwise match the whole string!)
      // greedy: true,
    },
  ],
  ...
  // sections
  // important: /\s(?:begsr|endsr|inzsr|exec-sql|end-exec)(?![\w-])/i,
  routine: {
    pattern: /\s(?:begsr|endsr|inzsr|exec-sql|end-exec)(?![\w-])/i,
    lookbehind: true,
  },

  keyword: [
    // main keywords
    /\s(?:IF|ENDIF|IFEQ|IFNE|IFGE|IFGT|IFLE|IFLT|SELECT|ENDSL|WHEN|OTHER|WHENEQ|WHENNE|WHENGE|WHENGT|WHENLE|WHENLT|ELSE|ELSEIF|ANDEQ|ANDNE|ANDGE|ANDGT|ANDLE|ANDLT|OREQ|ORNE|ORGE|ORGT|ORLE|ORLT)\b/i,
    // other keywords
    /\s(?:\*ON|\*OFF|%SUBST|%EOF|%SCAN|%FOUND|\*BLANKS|\*BLANK|\*ZEROES|\*ZERO|MOVE|MOVEA|MOVEL|CALL|PLIST|KLIST|PARM|ENTRY|CAT|EVAL|ADD|SUB|MULT|DIV|MVR|Z-ADD|Z-SUB|SETLL|SETGT|READ|READE|READP|READPE|WRITE|CHAIN|EXSR|ENDCS|CAS|CASEQ|CASNE|CASGE|CASGT|CASLE|CASLT|CABEQ|CABNE|CABGE|CABGT|CABLE|CABLT|GOTO|TAG|DO|ENDDO|DOU|DOW|DOUEQ|DOWEQ|DOUGE|DOWGE|DOUGT|DOWGT|DOULE|DOWLE|DOULT|DOWLT|CLEAR|RESET|LIKE|DEFINE|DTAARA|SETON|SETOFF|EXFMT|LEAVE|LEAVESR|ITER|RETURN)\b/i,
    /\s(?:select|from|where|and|or|update|merge|insert|date|case|end|group|by|order|by|distinct)\b/i,
  ],
